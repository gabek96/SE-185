...
#define X 3
#define CIRCLE 4
int noPresses(int a, int b, int c, int d);
int computerGuess();
...
while(userGuess(b1, b2, b3, b4)!=TRIANGLE){
	scanf(" %d, %d, %d, %d, %d", &t, &b1, &b2, &b3, &b4);	
}
	...
	while(activeGame==TRUE){
		...
		int btpress = noPresses(b1, b2, b3, b4);
		if(btpress != 1){
			printf("test\n");
		while(StillActive(t, timeGiven + startTime)==TRUE){
		scanf(" %d, %d, %d, %d, %d", &t, &b1, &b2, &b3, &b4);
		pressed = userGuess(b1, b2, b3, b4);
		printf("value in pressed %s \n ", pressed);
