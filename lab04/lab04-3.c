...
-----------------------------------------------------------------------------*/
#include <stdio.h>
#include <stdlib.h> //added this for rand functions
#include <time.h>
					//commented out below section 
/*-----------------------------------------------------------------------------
-                                Prototypes                                   -
------------------------------------------------------------------------------*/
char ask_to_play(int times_played);
void run_game(int computer_guess); // declared as a function
...
int main(int argc, char *argv[])
{
    ...
    while (prompt == 'y')     /* This line does not contain an error */
    {
        ...
        //prompt = ask_to_play(playd); added e in played
		prompt = ask_to_play(played);
    }
...
void run_game(int computer_number)
{
    ...
    //scanf("%c", &number); number is an int not a char
	scanf("%d", &number);
	int correct=0; //added this to fix error
    while (!correct)    /* This line does not contain an error */
    {
        if (number < 1 || number > 100)
        {
            printf("\nYour number is not within the correct range of numbers. Guess again\n  -> ");
        } //else if (number = computer_number) added another =
